...
$query2 = "
SELECT COUNT(glpi_tickets.id) AS tick, glpi_itilsolutions.solutiontypes_id AS tipo, glpi_tickets.$olaid AS id, glpi_olas.name
FROM glpi_tickets
...
JOIN glpi_olas ON glpi_tickets.$olaid = glpi_olas.id
WHERE glpi_tickets.is_deleted = '0'
AND glpi_tickets.date ".$datas."
".$entidade."
AND glpi_olas.type = 1
AND (glpi_tickets.status = 5 OR glpi_tickets.status = 6)
GROUP BY glpi_itilsolutions.solutiontypes_id, glpi_tickets.$olaid
...
<script type='text/javascript'>
    $(function () {		
        // Build the chart
        $('#graf_tipo_grupo_oltsa').highcharts({
            chart: {
