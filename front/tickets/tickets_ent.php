...
<html> 
	<head>
		<title> <?php echo $entidade_user; ?> - <?php echo __('Open Tickets','dashboard'); ?> </title>
...
if(isset($_REQUEST['order'])) {
	...
	switch($order1) {
		...
		echo "<th style='text-align:center;'>". __('Requester')."</th>
				<th style='text-align:center;'>". __('Technician')."</th>"; 
						
		echo "<th style='text-align:center;'><a href='tickets.php?&order=pa'>&nbsp<font size=2.5pt; font-family='webdings'style='color:#fff;'>&#x25BE;&nbsp;</font></a>". __('Priority')."<a href='tickets.php?&order=pd'><font size=2.5pt; font-family='webdings'style='color:#fff;'>&nbsp;&#x25B4;</font></a></th>";
					
		echo "<th style='text-align:center;'><a href='tickets.php?&order=sa'><font size=2.5pt; font-family='webdings'style='color:#fff;'>&#x25BE;&nbsp;</font></a>". __('Status')."<a href='tickets.php?&order=sd'><font size=2.5pt; font-family='webdings'style='color:#fff;'>&nbsp;&#x25B4;</font></a></th>";
				
		echo "<th style='text-align:center;'><a href='tickets.php?&order=pa'>&nbsp<font size=2.5pt; font-family='webdings'style='color:#fff;'>&#x25BE;&nbsp;</font></a>". __('Progress')."<a href='tickets.php?&order=pd'><font size=2.5pt; font-family='webdings'style='color:#fff;'>&nbsp;&#x25B4;</font></a></th>
		</tr>
		</thead>
		<tbody>";
...
while($row = $DB->fetchAssoc($result_cham)){ 
		...
		// get technician / Fornecedor	
		$sql_for = "SELECT glpi_suppliers_tickets.tickets_id AS id, glpi_suppliers.name As fname
		FROM `glpi_suppliers_tickets` , glpi_tickets, glpi_suppliers
		WHERE glpi_tickets.id = glpi_suppliers_tickets.`tickets_id`
		AND glpi_tickets.id = ". $row['id'] ."
		AND glpi_suppliers_tickets.`suppliers_id` = glpi_suppliers.id
		AND glpi_suppliers_tickets.type = 2 ";
				
		$result_for = $DB->query($sql_for);	
		$row_for = $DB->fetchAssoc($result_for);
		...
		if($show_tit != 0 || $show_tit == '') {	
			echo "<td style='vertical-align:middle; text-align:center; '><a href=../../../../front/ticket.form.php?id=". $row['id'] ." target=_blank > <span>" . $row['descri'] . "</span> </a></td>";
		}	
		...
		echo "<td style='vertical-align:middle; text-align:center; '><span >". $row_req['name'] ." ".$row_req['sname'] ."</span> </td>";					
					
		//Trás o técncico ou o fornecedor
		if(isset($row_tec['name'])) {		
			echo "<td style='vertical-align:middle; text-align:center; '><span >". $row_tec['name'] ." ".$row_tec['sname'] ." </span> </td>";
		} elseif(isset($row_for['fname'])) {		
			echo "<td style='vertical-align:middle; text-align:center; '><span >". $row_for['fname'] ." </span> </td>";
		} else {
			echo "<td style='vertical-align:middle; text-align:center; '><span ></span> </td>";
		}
		
		echo "<td style='vertical-align:middle; text-align:center; background-color:". $row_prio['value'] .";'>" . $prio_name . "</td>";
					
		echo "<td style='vertical-align:middle; text-align:center; '><span><img src=../../../../pics/".$status1.".png /> ".Ticket::getStatus($row['status'])."</span ></td>";
