...
if(!isset($_POST["sel_date"])) {
	$id_date = $_GET["sel_date"] ?? null;
}

else {
	$id_date = $_POST["sel_date"] ?? null;
}
...
<html>
<head>
<title> <?php echo $entidade_user; ?> - <?php echo __('Tickets','dashboard') .'  '. __('by Date','dashboard') ?> </title>
...
<div id='content' >
	<div id='container-fluid' style="margin: <?php echo margins(); ?> ;">
		...
		// do select
		$post_date = isset($_POST["sel_date"]) ? $_POST["sel_date"] : '';$post_date = $_POST["sel_date"] ?? null;

		if (!empty($post_date) && $post_date != "0") {
		...
		while($row = $DB->fetchAssoc($result_cham)){
			...
			if (!$row_tec) {
				$row_tec['name'] = "Técnico não atribuído";
				$row_tec['sname'] = "";
			}
			...
			<tr style='font-weight:normal;'>
				...
				<td style='vertical-align:middle;'> ". (isset($row_user['name']) ? $row_user['name'] : '') ." ". (isset($row_user['sname']) ? $row_user['sname'] : '') ." </td>
				<td style='vertical-align:middle;'> ". (isset($row_tec['name']) ? $row_tec['name'] : '') ." ". (isset($row_tec['sname']) ? $row_tec['sname'] : '') ." </td>
