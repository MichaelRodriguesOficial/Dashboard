...
<html>
<head>
<title> <?php echo $entidade_user; ?> - <?php echo __('Tickets', 'dashboard') .'  '. __('by Location', 'dashboard') ?> </title>
...
if($con == "1") {{
  // predefine o mês atual
  $mes_atual = date("m");
  $ano_atual = date("Y");
  
  if(!isset($_POST['date1'])) {
    $data_ini2 = $ano_atual . "-" . $mes_atual . "-01";
    $data_fin2 = $ano_atual . "-" . $mes_atual . "-" . date("t", strtotime($ano_atual . "-" . $mes_atual . "-01"));
  } else {
    $data_ini2 = $_POST['date1'];
    $data_fin2 = $_POST['date2'];
  }
